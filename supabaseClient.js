(function () {
  ...
  async function pullTabla(tabla) {
    try {
      const { data } = await apiRequest("pull_table", { query: { table: tabla } });
      if (Array.isArray(data)) {
        localStorage.setItem(localKey(tabla), JSON.stringify(data));
      }
    } catch (error) {
  ...
  async function pullAll() {
    try {
      const { data } = await apiRequest("pull_all", {
        query: { tables: TABLAS.join(",") },
      });
      TABLAS.forEach((tabla) => {
        if (Array.isArray(data?.[tabla])) {
          localStorage.setItem(localKey(tabla), JSON.stringify(data[tabla]));
        }
      });
    } catch (error) {
