...
function renderVistaHoy() {
  ...
  if (hoyAuditoria && window.supabaseSync?.fetchAuditRecent) {
    window.supabaseSync.fetchAuditRecent().then((rows) => {
      ...
      hoyAuditoria.innerHTML = `<strong>Auditoría reciente:</strong> ${top.length ? top.map((r) => `[${r.action}] ${r.table_name || "-"} #${r.app_id || "-"} por ${r.actor || "sistema"}`).join(" · ") : "Sin movimientos recientes"}`;
    });
  }
...
function actualizarKpiResumen() {
  ...
  el.innerHTML = `
    ...
    <div class="kpi-item"><strong>Tareas activas</strong><br>${tareas.length}</div>
    <div class="kpi-item"><strong>Vencidas</strong><br>${vencidas}</div>
    <div class="kpi-item"><strong>Terminadas</strong><br>${terminadas}</div>
    <div class="kpi-item"><strong>Audiencias</strong><br>${audiencias.length}</div>
  `;
}
